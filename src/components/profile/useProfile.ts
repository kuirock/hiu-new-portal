...
export function useProfile(targetUserId: string, currentUserId: string, initialProfile: any) {
    ...
    useEffect(() => {
        if (targetUserId) fetchFollowCounts();
    }, [targetUserId]);
    ...
    const fetchProfile = async () => {
        ...
        try {
            // ★★★ ここが修正ポイント！ 'users' ではなく 'profiles' にする！ ★★★
    ...
    return {
        profile, loading, isFollowing, isBlocked, followersCount, followingCount,
        isListOpen, setIsListOpen, listType, setListType, isPostModalOpen, setIsPostModalOpen, feedRef,
        handleFollow, handleBlock,
        refreshProfile: fetchProfile // ★これを返す！
    };
}